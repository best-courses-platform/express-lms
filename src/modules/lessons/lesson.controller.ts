...
export const createLessonForCourse: RequestHandler = async (req, res) => {
...
export const listLessons: RequestHandler = async (_req, res) => {
...
export const checkLessonAccess: RequestHandler = async (req, res) => {
...
export const uploadLessonFile: RequestHandler = async (req, res) => {
...
export const deleteLessonFile: RequestHandler = async (req, res) => {
  if (!isAuthenticatedRequest(req)) {
    throw new UnauthorizedError(LESSON_MESSAGES.ERROR.UNAUTHORIZED);
  }
...
export const deleteLessonResource: RequestHandler = async (req, res) => {
  if (!isAuthenticatedRequest(req)) {
    throw new UnauthorizedError(LESSON_MESSAGES.ERROR.UNAUTHORIZED);
  }
...
export const LessonController = {
  createLessonForCourse: [validate(createLessonForCourseSchema), asyncHandler(createLessonForCourse)],
  listLessons: asyncHandler(listLessons),
  getLesson: [validate(idParamSchema), asyncHandler(getLesson)],
  updateLesson: [validate(updateLessonSchema), asyncHandler(updateLesson)],
  deleteLesson: [validate(idParamSchema), asyncHandler(deleteLesson)],
  getLessonsByCourse: [validate(courseIdParamSchema), asyncHandler(getLessonsByCourse)],
  checkLessonAccess: [validate(accessCheckSchema), asyncHandler(checkLessonAccess)],
  uploadLessonFile: [validate(uploadFileSchema), asyncHandler(uploadLessonFile)],
  deleteLessonFile: [validate(deleteFileSchema), asyncHandler(deleteLessonFile)],
  deleteLessonResource: [validate(deleteResourceSchema), asyncHandler(deleteLessonResource)],
};
