...
export const ENTITY_VALIDATION = {
  ...
  FIELD_TOO_LONG: (field: string, entity: string, maxLength: number) =>
    `${field} ${entity} не должно превышать ${maxLength} символов`,
  FIELD_TOO_SHORT: (field: string, entity: string, minLength: number) =>
    `${field} ${entity} должно содержать не менее ${minLength} символов`,
} as const;
