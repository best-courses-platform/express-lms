...
// Пул перегружен (см. password-hasher.ts — очередь piscina переполнена) — сервер жив и
// работает нормально, просто временно не может принять именно эту дорогую CPU-bound задачу
// прямо сейчас. Отдельный от InternalError класс: клиенту стоит повторить запрос чуть позже
// (retry-able), это не "сломанный сервер" и не "неверные входные данные" — 503, а не 500/4xx.
