...
registry.registerPath({
  ...
  description:
    'Тело — ровно одно из двух: { email } (письмо не пришло) или { token } (ссылка из письма просрочена — ' +
    'аккаунт находится по токену, вводить email не нужно). ' +
    'Всегда отвечает 200, даже если email/токен не найдены, аккаунт уже подтверждён или сработала серверная ' +
    'пауза между письмами (60 секунд) — иначе ответ бы палил факт существования аккаунта (user enumeration).',
